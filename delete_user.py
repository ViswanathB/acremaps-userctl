...
class DeleteUser(object):
    ...
    def __mark_user_deleted(self, email: str) -> None:
        try:
            ...
            delete_email_query = f"UPDATE nobel_{self.env}.farm_user SET email = '{str(uuid.uuid4())}' WHERE email ='{email}';"
            # fmt: on

            self.__run_postgres_update(self._db_conn, kratos_id_null_query)
            self.__run_postgres_update(self._db_conn, set_username_query)
            self.__run_postgres_update(self._db_conn, update_name_query)
            self.__run_postgres_update(self._db_conn, delete_email_query)
    ...
    def __delete_kratos_user_as_admin(self, kratos_id: str) -> str:
        req_url = "https://kratos-admin.acremaps.one/admin/identities/"
    ...
    def __get_kratos_id(self, email: str) -> Union[str, None]:
        ...
        records = self.__run_postgres_get(self._kratos_conn, get_kratos_email_query)
        if len(records) == 1:
            return records[0][0]
    ...
    def __process_user(self, email: str):
        if self.__is_user_present(self._db_conn, email):
            self.total_users += 1
